...
public class VehicleTireEffects : MonoBehaviour
	...
	void UpdateTireMarks (WheelData wheelData, TireFxData fxData)
		...
		if (fxData.lastMarksIndex != -1 && wheelData.visualGrounded && fxData.marksDelta < updateInterval)
			{
		...
		if (!wheelData.visualGrounded || wheelData.visualHitCollider.attachedRigidbody != null)
			{
		...
		if (marksRenderer != null)
			...
			fxData.lastMarksIndex = marksRenderer.AddMark(
				wheelData.visualHit.point - wheelData.transform.right * wheelData.collider.center.x + wheelData.velocity * deltaT,
				wheelData.visualHit.normal,
				pressureRatio,
				skidRatio,
				tireWidth,
	...
	void UpdateTireParticles (WheelData wheelData, TireFxData fxData)
		...
		if (particleEmitter != null)
			{
			Vector3 position = wheelData.visualHit.point + wheelData.transform.up * tireWidth * 0.5f;
