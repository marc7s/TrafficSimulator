...
public class AStarNode : System.IComparable<AStarNode>, System.IEquatable<AStarNode>
 {
    ...
    public bool Equals(AStarNode other) {
        return string.Equals(GraphNode.Node.uID,other.GraphNode.Node.uID);
    }
}
    ...
    public static class Navigation
    {
        ...
        public static Stack<string> GetPathToNode(GraphNode startNode, GraphNode endNode)
        {
            ...
            while (toBeSearched.Count > 0)
            {
                ...
                foreach (GraphEdge edge in current.GraphNode.Edges)
                {
                    ...
                    if (!costMap.ContainsKey(edge.EndNode.Node.uID) || costToNode < costMap[edge.EndNode.Node.uID])
                    {
                        // Update the cost
                        costMap[edge.EndNode.Node.uID] = costToNode;
        ...
        {
            return Vector3.Distance(node.Node.transform.position, targetNode.Node.transform.position);
        }
        ...
        private static Stack<string> GetPathToNode(AStarNode node)
        {
            ...
            {
                path.Push(node.GraphNode.Node.uID);
